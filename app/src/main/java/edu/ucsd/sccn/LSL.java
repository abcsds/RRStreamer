/*
 * Vendored from https://github.com/labstreaminglayer/liblsl-Java
 *   - Originally taken from mvidaldp/liblsl-android-builder v1.16.2 (matches
 *     the native liblsl.so versions we package; see flake.nix `liblslJniLibs`).
 *   - Package rewritten from `liblsl.android.builder` to `edu.ucsd.sccn` so
 *     downstream LSL examples that import `edu.ucsd.sccn.LSL.*` work as-is.
 *   - Local patches:
 *       * StreamOutlet(info, chunk_size) and StreamOutlet(info, chunk_size,
 *         max_buffered): the upstream source unconditionally throws
 *         IOException after creating the outlet. Fixed to throw only when
 *         the C-side handle is null, matching StreamOutlet(info).
 *
 * If you bump the native libs, re-vendor this file from the matching upstream
 * commit and re-apply the patches above.
 */
